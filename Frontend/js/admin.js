...
function renderDashboard() {
  ...
  const monthlyRevenue = goldMembers * 100; // mock: $100/gold member
  ...
  const peakBars = peakHoursData.map(d => {
    ...
    return `<div class="bar-wrap">
      <div class="bar" style="height:${heightPct}%;" title="${d.count} members"></div>
  ...
  const points = retentionData.map((d, i) => {
    const x = 40 + i * 90;
    const y = 140 - (d.rate / maxRetention) * 120;
    return `${x},${y}`;
  }).join(' ');
  const retentionDots = retentionData.map((d, i) => {
    const x = 40 + i * 90;
    const y = 140 - (d.rate / maxRetention) * 120;
    return `<circle cx="${x}" cy="${y}" r="4" fill="#6366f1"/><title>${d.month}: ${d.rate}%</title>`;
  }).join('');
  const xLabels = retentionData.map((d, i) => {
    const x = 40 + i * 90;
    return `<text x="${x}" y="158" text-anchor="middle" font-size="11" fill="#94a3b8">${d.month}</text>`;
  }).join('');
  const yLabels = [0, 25, 50, 75, 100].map(v => {
    const y = 140 - (v / maxRetention) * 120;
    return `<text x="28" y="${y + 4}" text-anchor="end" font-size="11" fill="#94a3b8">${v}</text>
    <line x1="35" y1="${y}" x2="550" y2="${y}" stroke="#e2e8f0" stroke-width="1"/>`;
  }).join('');
...
function renderMembersTab() {
  const rows = members.map(m => {
    // Plan badge: Gold = dark/black pill, Silver = plain text (matches PDF)
    const planCell = m.plan === 'Gold'
      ? `<span class="plan-badge-gold">Gold</span>`
      : `<span class="plan-text-silver">Silver</span>`;

    // Status pill: active = dark pill, banned = red pill (matches PDF)
    const statusCell = `<span class="status-pill-${m.status}">${m.status}</span>`;

    // Action buttons with SVG icons (matches PDF rounded square buttons)
    return `
    <tr>
      ...
      <td>${statusCell}</td>
      <td>${m.expiry}</td>
      <td>Weight: ${m.weight}kg, BMI: ${m.bmi}</td>
  ...
  <div class="table-wrap">
    <table class="data-table">
      <thead><tr>
        ...
        <th>Membership Expiry</th><th>Health</th><th>Actions</th>
      </tr></thead>
      <tbody id="members-tbody">${rows}</tbody>
    </table>
  </div>`;
}
...
function openRegisterMember() {
  showModal(`
    ...
    <div class="form-group"><label>Name</label><input id="m-name" class="form-input" placeholder="Full name"/></div>
    <div class="form-group"><label>Contact (Email)</label><input id="m-contact" class="form-input" placeholder="email@example.com"/></div>
    ...
    <div class="form-group"><label>Expiry Date</label><input id="m-expiry" class="form-input" type="date"/></div>
...
function saveNewMember() {
  ...
  const bmi = parseFloat(document.getElementById('m-bmi').value);
  const expiryRaw = document.getElementById('m-expiry').value;

  if (!name || !contact || !rfid || !expiryRaw) { showToast('Please fill all required fields.', 'error'); return; }
  if (members.find(m => m.rfid.toUpperCase() === rfid.toUpperCase())) { showToast('RFID already exists.', 'error'); return; }

  const expDate = new Date(expiryRaw);
  const expiry = `${expDate.getMonth()+1}/${expDate.getDate()}/${expDate.getFullYear()}`;

  members.push({
    id: rfid.toUpperCase(), rfid: rfid.toUpperCase(), name, contact, plan,
    status: 'active', expiry, weight: weight || 0, bmi: bmi || 0,
...
function openEditMember(id) {
  ...
  showModal(`
    ...
    <div class="form-group"><label>Name</label><input id="em-name" class="form-input" value="${m.name}"/></div>
    <div class="form-group"><label>Contact</label><input id="em-contact" class="form-input" value="${m.contact}"/></div>
    <div class="form-group"><label>Plan</label>
    ...
    <div class="form-group"><label>Expiry (MM/DD/YYYY)</label><input id="em-expiry" class="form-input" value="${m.expiry}"/></div>
    <div class="modal-actions">
      <button class="btn-secondary" onclick="closeModal()">Cancel</button>
...
function renderTrainersTab() {
  const monthOptions = ['all','2026-1','2026-2','2026-3','2026-4','2026-5','2026-6','2026-7','2026-8','2026-9','2026-10','2026-11','2026-12'];
  const monthLabels = { all: 'All Time', '2026-1':'Jan','2026-2':'Feb','2026-3':'Mar','2026-4':'Apr','2026-5':'May','2026-6':'Jun','2026-7':'Jul','2026-8':'Aug','2026-9':'Sep','2026-10':'Oct','2026-11':'Nov','2026-12':'Dec' };
  const opts = monthOptions.map(k => `<option value="${k}" ${trainerEarningsFilter===k?'selected':''}>${monthLabels[k]}</option>`).join('');

  const rows = trainers.map(t => {
    const earnings = getTrainerEarnings(t, trainerEarningsFilter);
    const sessions = getFilteredSessions(t, trainerEarningsFilter);
    const label = trainerEarningsFilter === 'all' ? 'All Time' : monthLabels[trainerEarningsFilter];
    return `<tr>
      <td><strong>${t.name}</strong></td>
      <td>${t.specialization}</td>
      <td>$${t.ratePerSession}</td>
      <td>${sessions}</td>
      <td class="earnings-green">$${earnings} <span class="filter-label">(${label})</span></td>
      <td class="action-cell">
  ...
  <div class="tab-header">
    ...
    <div class="tab-header-right">
      ...
      <select id="earnings-filter" class="form-select" onchange="changeEarningsFilter(this.value)">${opts}</select>
...
function openOnboardTrainer() {
  ...
  showModal(`
    ...
    <div class="form-group"><label>Name</label><input id="t-name" class="form-input" placeholder="Full name"/></div>
...
function saveNewTrainer() {
  ...
  if (trainers.find(t => t.rfid.toUpperCase() === rfid.toUpperCase())) { showToast('Trainer ID already exists.', 'error'); return; }
  trainers.push({
    id: rfid.toUpperCase(), rfid: rfid.toUpperCase(), name, specialization: spec,
    ratePerSession: rate, loggedIn: false, clockInTime: null,
...
function openEditTrainer(id) {
  ...
  if (!t) return;
  const specOpts = WORKOUT_PROGRAMS.map(p => `<option value="${p}" ${t.specialization===p?'selected':''}>${p}</option>`).join('');
  showModal(`
    ...
    <div class="form-group"><label>Rate per Session ($)</label><input id="et-rate" class="form-input" type="number" value="${t.ratePerSession}"/></div>
    <div class="modal-actions">
      <button class="btn-secondary" onclick="closeModal()">Cancel</button>
